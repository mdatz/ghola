        ...
        try{
            ...
            try{
                ...
                if(includeSubobjects && includeSubobjects.toLowerCase() === 'true') {
                    await Profile.init();
                    conversationRecords = await ConversationRecord.find(query).limit(queryLimit).skip(queryPage * queryLimit).populate('profileId', 'name description imageUrl messageCount').select('-userId -__v');
                } else {
                    conversationRecords = await ConversationRecord.find(query).limit(queryLimit).skip(queryPage * queryLimit).select('-userId -__v');
                }
